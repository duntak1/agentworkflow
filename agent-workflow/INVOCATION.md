...
**本文件与 `scripts/*.sh` 适用于任意 AI 编码工具**（Claude Code、OpenAI Codex、GitHub Copilot、Cursor、Windsurf、Cline、Continue、QoderWork / Qoder、TraeIDE、Lingma、OpenClaw、qclaw、网页对话等）。
...
| 代码地图 | `aw code-map build|query|impact|affected|gate` 生成 / 查询 `docs/context/CODE_MAP.md`，先定位模块、入口、Symbol、路由和测试，再决定读取范围；默认由 `aw context plan`、`aw task start/complete`、`aw watch index`、`aw gate pre-commit` 自动刷新 |
...
| Agent 运行绑定 | `aw agents bind <agent-id> --runtime qoderwork|trae|lingma|openclaw|qclaw` 记录 Agent 实际运行工具、provider、workspace、接口和同步模式 |
| Agent 绑定查询 | `aw agents bindings` / `aw agents list --bindings` 查看注册 Agent 的 runtime / tool binding 状态 |
...
| 多 Agent 协作 | `aw agents assign|handoff|review ...` 记录任务分配、文件边界、交接和评审结论；`aw agents gate` 检查阻断评审、未注册 Agent 引用、缺少 runtime binding 和路径重叠，`--strict` 可阻断冲突 |
| 多 Agent 严格门禁 | `aw agents gate --strict` 发现未注册 Agent、缺少 runtime binding 或 allowed paths 重叠时阻断，要求先 register、bind、handoff 或重新分配文件边界 |
...
当工程师说“执行研发任务 / 开始开发 / 做下一个任务”时，Agent 的第一步必须是输出或执行 `aw next` 与 `aw task brief <AT-T>`，然后像真实研发一样追问需求、边界、验收、异常态、联动、非目标和风险。工程师明确确认前，不得写业务代码，不得生成编码提示，不得调用 `aw paste task`。确认后必须记录 `aw task confirm <AT-T> "已确认：范围=...；验收=...；非目标=..."`，再执行 `aw context plan`、`aw context gate`、`aw task start`；其中 `context plan` 和 `task start` 会自动刷新 / 检查代码地图。
...
| QoderWork / Qoder | `.qoderwork/rules/agent-workflow.md` | `aw adapters --qoderwork` |
| TraeIDE | `.trae/rules/agent-workflow.md` | `aw adapters --trae` |
| Lingma / 通义灵码 | `.lingma/rules/agent-workflow.md` | `aw adapters --lingma` |
| OpenClaw | `.openclaw/agent-workflow.md` | `aw adapters --openclaw` |
| qclaw | `.qclaw/agent-workflow.md` | `aw adapters --qclaw` |
