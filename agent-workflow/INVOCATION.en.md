...
5. Before every AT-T coding prompt, the hard gate is `aw task confirm` with scope / acceptance / non-goals → auto code-map refresh/query → `aw context plan` → `aw context gate` → `aw task start` → `aw paste task`. If any step is missing, do not read broadly, generate a coding prompt, or edit business code.
