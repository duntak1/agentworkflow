...
```text
Project Scan → 阶段确认 → 项目类型/构建目标确认 → 必要时同步中心就绪 → Reference/基线 → DSL 已审 → Plan 可执行 → confirm → AT-T 任务 → verify → handoff
```

| 能力 | 解决的问题 |
|------|------------|
| `reference/manifest.yaml` | 材料来源可追踪，避免编造路径 |
| `aw project scan|gate` | 先扫描项目内容判断全新/已有项目；未扫描、未确认阶段或 fullstack 未建同步中心时阻断 Plan |
