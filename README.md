...
| `aw task start|blocked|complete|checkpoint` | 任务状态可追踪；完成必须来自已 start 任务，并补 Git / compact / handoff / FILE_INDEX 检查点 |
...
| `aw pm start|init|intake-check|design|dispatch|dashboard|gate` | PM Agent 产品全生命周期：同步中心资料、Pencil、DSL/Plan、三端任务派发、看板、生命周期 Gate |
