...
| Harness control | `aw pm`, `aw gate`, `aw contract`, `aw vcs`, `aw agents claim`, `aw score`, and `aw recover` make PM lifecycle control, hooks, API contracts, PR/MR/CRs, agent locks, delivery scoring, and recovery checkable |
...
| PM intake | `aw pm intake-check --write` scans PRD/UI/tech/API/business/Pencil materials and records missing references before DSL generation |
| PM design | `aw pm design import --file <file.pen> --req <REQ-ID>` or `aw pm design link --req <REQ-ID> --source <path>` registers Pencil files; `.pen` files must be exported/screenshot via Pencil tooling before frontend/backend agents read them |
| PM plan | After reviewed shared DSL is in `project-harness/global/dsl/`, run `aw pm plan --write`; PM/engineer must review `global/plans/*` before dispatch |
| PM dispatch/dashboard | After Plan approval, run `aw pm dispatch --write`, then `aw pm assignments --role frontend|admin|backend`, and `aw pm dashboard --write` to maintain three-side task boards and PM-readable progress; dispatch also refreshes the PM dashboard |
| PM lifecycle gate | `aw pm gate` checks product brief, backlog, design freeze, tech design, integration matrix, test strategy, UAT, release checklist, post-release review, and lifecycle board; `--strict` blocks pending lifecycle statuses |
| Plan | `aw plan <dsl>` → `aw paste plan-write` or `aw plan apply` → `aw approve plan`; if PM sync center is configured, approval must guide `aw pm dispatch --write` before implementation agents claim frontend/admin/backend tasks → `aw check plan` |
...
| Dev | `aw status` → `aw next` → `aw task brief <id>` → discuss requirements → `aw task confirm <id> "已确认：范围=...；验收=...；非目标=..."` → `aw context plan --task <id>` → review allowed files → `aw context gate --task <id>` → `aw task start <id>` → `aw paste task` → `aw task complete <id>` → `aw compact` / `aw file-index` / `aw task checkpoint`; when sync is configured, task start auto-runs `sync pull + sync gate`, task complete auto-publishes a complete event, and task blocked auto-publishes a block event |
...
| File index | Update `docs/FILE_INDEX.md` when adding/deleting/renaming business files so human engineers can find files to review or hand-edit; `aw gate file-index` blocks added/deleted/renamed business files without an index refresh |
...
| Completion checkpoint | After `aw task complete`, run `aw compact "完成 <id>" --write --snapshot`, `aw file-index`, then `aw task checkpoint <id> --git yes|no --reason "..." --handoff --compact --file-index`; `aw gate task` blocks completed tasks without this checkpoint |
...
| Automation helpers | `aw context enrich`, `aw verify --affected`, `aw contract diff --write`, `aw vcs fill`, and `aw watch index` automate context enrichment, affected validation, contract diffs, PR/MR/CR drafts, and index refresh |
| Auto sync hooks | With `docs/sync/SYNC_CONFIG.md` configured, `aw task start` auto-runs `aw sync pull` and `aw sync gate --task <id>`; `aw task complete` auto-runs `aw sync event --type complete`; `aw task blocked` auto-runs `aw sync event --type block`; `aw contract change` auto-runs a contract sync event. Set `AW_SYNC_AUTO=0` or `AW_CONTRACT_AUTO_SYNC=0` only for explicit exceptions |
...
| VCS PR/MR/CR loop | `aw vcs branch|fill|create|review|gate` records branch policy, review checklist, outcomes, Contract/Score/Release readiness, and rollback notes for GitHub, GitLab, Bitbucket, Gitee, GitCode, Gitea, Forgejo, GitLab CE, Gerrit, and Codeup |
...
| Cross-project sync | For split frontend/backend repos, configure a shared harness with `aw sync init <dir> --project <name> --agent <name>`; run `aw sync pull` and `aw sync gate --task <id>` before starting dependent work, and `aw sync push --task <id>` after updating contracts, handoff, bugs, requirements, or verification |
| PM-managed sync center | PM Agent owns `project-harness/global/` as the product fact source; implementation agents treat pulled PM files as requirements/context and only update global DSL/Plan/dispatch through PM confirmation |
...
7b. Product新增需求、设计稿变更、三端派发变更 must enter the PM queue first with `aw pm change` or `aw pm design change`, then update PM `requirements/BACKLOG.md`, DSL, Plan, dispatch board, and dashboard before any implementation agent changes code.
...
15a. Three-side coordination uses PM dispatch plus contracts: frontend, admin frontend, and backend agents read `global/dispatch/*_ASSIGNMENTS.md` and `global/contracts/INTEGRATION_MATRIX.md`; cross-side interface ambiguity must be recorded as PM change, contract change, or blocker, not solved by private chat only.
...
17. Before PR/MR/CR or release handoff, run `aw gate pr`, `aw vcs gate`, `aw contract gate`, and `aw score record`; recover with `aw recover ...` instead of guessing when a gate fails.
17a. A task cannot be completed unless it was started with `aw task start`; `aw task done` verifies by default. A completed task must get a completion checkpoint before the next large handoff: Git decision, compact snapshot, handoff evidence, and FILE_INDEX refresh.
...
22. Pencil design files are formal requirement inputs. Store `.pen` sources under `global/references/design/pencil/source/`, exports under `exports/`, screenshots under `screenshots/`, and design changes in `DESIGN_CHANGELOG.md`. Do not parse encrypted `.pen` files as plain text; use Pencil tooling or exported artifacts.
23. Product lifecycle gates are real gates: unconfirmed product brief blocks DSL, unreviewed requirements block formal Plan, unfrozen design blocks formal frontend work, unconfirmed tech design blocks formal backend work, unconfirmed API contract blocks integration, missing test/UAT/release/postmortem artifacts block completion, release, or project closure as applicable.
